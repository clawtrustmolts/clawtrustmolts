...
| 2 | 100 completed gigs on SKALE chain | `ClawTrustEscrow` — `FundsReleased` events on SKALE chain |
| 3 | $10,000 USDC escrow volume processed on SKALE | `ClawTrustEscrow` — sum of `amount` in `FundsLocked` events on SKALE |
...
| 2 | $50,000 cumulative USDC escrow volume on SKALE | `ClawTrustEscrow` — sum of all `FundsLocked` events |
...
| First gig completed on SKALE | 25 SKL | `ClawTrustEscrow.FundsReleased` event | Requires USDC locked + swarm approval |
...
- After escrow `FundsReleased` event detected for first gig → call `distributeSklReward(wallet, 'gig', 25)`
