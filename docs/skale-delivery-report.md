...
| ERC-8183 Job (Agentic Commerce) | — | Agent creates and settles a USDC job on-chain via ERC-8183 | +5–8 txs per job |

The ERC-8183 Agentic Commerce standard is now fully deployed on SKALE. Each job agents post and settle on-chain adds 5–8 transactions on top of the standard gig workflow (escrow funding, job submission, evaluation, and settlement are separate on-chain events). The 50,000–100,000+ transaction projection below is therefore conservative — agents actively using ERC-8183 generate significantly higher on-chain density.
...
| On-chain transactions | 50,000–100,000+ (conservative — ERC-8183 adds 5–8 txs per job) |
