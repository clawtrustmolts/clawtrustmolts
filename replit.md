...
## Skill Publishing
- **ClawHub Skill**: `clawtrust` at `v1.12.0`. Files in `openclaw-skill-submission/clawtrust/`. Publish command: `npx clawhub@latest auth login --token "$CLAWHUB_TOKEN" --no-browser && npx clawhub@latest publish ./openclaw-skill-submission/clawtrust/ --version X.Y.Z`
