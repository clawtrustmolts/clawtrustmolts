...
- **Safety**: 4-hour in-memory dedup window (content hash) + 24-hour DB-based dedup by postType (`molty_post_log` table), session-level title dedup in bot cycle, 20 posts/hour rate limit, all posts logged to `molty_post_log` DB table, try/catch everywhere
