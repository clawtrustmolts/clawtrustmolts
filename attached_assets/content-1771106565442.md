...
# Everclaw — Morpheus AI Inference
...
by [@DavidAJohnston](https://clawhub.ai/u/DavidAJohnston)
...
[View report →](https://www.virustotal.com/gui/file/25f0fdaeb0c35cc69473e6566dc4210fae6963e178c460b30e2194b42b3e74cd)
...
The skill's functionality (local router, staking wallets, payment client, security tooling) broadly matches its description, but there are multiple red flags: it asks you to run remote installers (curl \| bash), includes wallet/payment code and external network endpoints without declaring required credentials, and contains third‑party network calls — proceed with caution and audit the installer/scripts before running.Details ▾

ℹ

Purpose & Capability

Name/description (Morpheus inference, staking MOR, local proxy/router, wallet management, payments) reasonably explains the included Node scripts and shell tools; requiring curl and node is coherent. However: the SKILL metadata declares no required env vars/credentials while the runtime docs and scripts clearly expect a wallet private key, RPC endpoint (ETH\_NODE\_ADDRESS), and other sensitive config — that mismatch is unexpected and should have been declared.

!

Instruction Scope

The SKILL.md instructs users to run remote installers (curl https://raw.githubusercontent.com/... \| bash) and to run many local scripts that will create files in ~/morpheus and ~/.clawdbot, manage wallets, perform swaps, and contact external services. The included security tools also read/modify local config (e.g., audit.py can chmod files) and the HiveFence client may send reports to a 3rd‑party API. These instructions go beyond simple API calls and can read/write local files, modify permissions, and transmit data externally — all of which are high-impact and require explicit, declared justification.

!

Install Mechanism

There is no registry 'install' spec, but the SKILL.md recommends piping a raw.githubusercontent.com install script into bash (curl \| bash). That is high-risk: arbitrary remote code execution from an external repo. The skill also instructs downloading binary releases from GitHub and extracting them to ~/morpheus. Those are legitimate for this use case but are still medium–high risk and should be audited before execution.

!

Credentials

The skill performs wallet management, staking, RPC access and payment operations, and yet requires.env and primary credential are empty in the registry metadata. The SKILL.md references WALLET\_PRIVATE\_KEY, ETH\_NODE\_ADDRESS and other sensitive config but does not declare them as required credentials. The HiveFence client and other components contact third‑party endpoints (a workers.dev domain) which could receive reports or data. Asking for or using private keys and payment capabilities without declaring them is disproportionate and opaque.

ℹ

Persistence & Privilege

The skill does not request always:true and does not declare automatic persistent inclusion. However, the documented components (proxy-router, Gateway Guardian with 'self-healing restart') expect to run persistent local services and may create/modify files under ~/.morpheus and ~/.clawdbot and change permissions via audit scripts. Those behaviors are typical for a local inference/router product but deserve caution because they grant persistent local presence and the ability to restart services.

Scan Findings in Context

\[ignore-previous-instructions\] expected: The prompt-injection phrase appears in the included Prompt Guard documentation and detection patterns — expected because the security tool lists examples of attack strings. It likely triggered the pre-scan regex but is not necessarily malicious here.

\[you-are-now\] expected: Also appears in the prompt-guard docs as an example of role-manipulation/jailbreak patterns. Presence is expected for a project whose purpose is prompt-injection detection, but the phrase in SKILL.md could also be used to try to influence downstream agents; treat carefully.

What to consider before installing

Key things to consider before installing or running this skill:

\- Do NOT run curl \| bash on the installer without reviewing it first. Download the install script (or the repo) and inspect the contents of scripts/install-everclaw.sh and any scripts it downloads before executing.
\- The skill will ask you to manage a wallet and stake tokens. Never paste your private key into a script or env var you haven't audited. Prefer using a hardware wallet or a platform-specific secure store and confirm how the scripts access keys (the repo claims macOS Keychain only — that is macOS-specific and may not work elsewhere).
\- The skill makes outbound network calls (Morpheus network, HiveFence API on a personal workers.dev domain). Understand what data may be reported or fetched (threat reports, telemetry). If you require privacy, run the software in an isolated environment (VM, container, or dedicated machine) and monitor outbound traffic.
\- Audit scripts that modify local permissions or auto-fix issues (audit.py.fix\_issues uses chmod) before running them with auto-fix enabled. Prefer running read-only audits first.
\- Because the registry metadata doesn't declare required credentials yet the skill needs RPC endpoints and wallet access, ask the publisher (or inspect SKILL.md and scripts) to list required env vars/credentials explicitly in the skill manifest.
\- If you lack the ability to review code, consider testing in a sandboxed environment, or decline installation until the project provides reproducible, signed releases and a clear, minimal install path (avoid piping remote scripts to shell).

Like a lobster shell, security has layers — review code before you run it.

Current version **v0.9.2**

[Download zip](https://wry-manatee-359.convex.site/api/v1/download?slug=everclaw-inference)
...
_Powered by [Morpheus AI](https://mor.org/)_
...
2. **Active** → Send inference requests using session\_id header
...
| Error | Fix |
| --- | --- |
| `session not found` | Use session\_id/model\_id as HTTP **headers**, not body fields |
...
| `model_not_found` | Requested model not in MODEL\_MAP |
...
OpenClaw supports **multiple API keys per provider** with automatic rotation. When one key's credits run out (billing error), OpenClaw disables _that key only_ and rotates to the next one — same model, fresh credits. This is the single most effective way to prevent downtime.
...
2. **Billing disable:** When a key returns a billing/credit error, that _profile_ is disabled with exponential backoff (starts at 5 hours). Other profiles for the same provider remain active.
...
1. **Key rotation within Venice** — Key 1 credits exhausted → billing disable on _that profile only_ → immediately rotates to Key 2 → Key 3 → etc. Same model, fresh credits.
...
security/prompt-guard/ARCHITECTURE.md13 KB

security/prompt-guard/requirements.txt12 B

security/prompt-guard/CHANGELOG.md3.9 KB

security/prompt-guard/blog/how-i-secured-my-ai-agent.md4.7 KB

security/prompt-guard/README.md4.9 KB

security/prompt-guard/scripts/audit.py12 KB

security/prompt-guard/scripts/hivefence.py12 KB

security/prompt-guard/scripts/analyze\_log.py7.9 KB

security/prompt-guard/scripts/detect.py54 KB

security/prompt-guard/SKILL.md16 KB

security/prompt-guard/config.example.yaml1.8 KB

security/prompt-guard/SECURITY.md2.2 KB

security/bagman/\_meta.json125 B

security/bagman/references/session-keys.md11 KB

security/bagman/references/delegation-framework.md14 KB

security/bagman/references/secure-storage.md7.1 KB

security/bagman/references/leak-prevention.md5.4 KB

security/bagman/references/prompt-injection.md8.2 KB

security/bagman/references/prompt-injection-defense.md8.2 KB

security/bagman/SKILL.md14 KB

security/clawdstrike/references/network.md1.4 KB

security/clawdstrike/references/threat-model.md1.0 KB

security/clawdstrike/references/config-keys.md2.0 KB

security/clawdstrike/references/supply-chain.md1.7 KB

security/clawdstrike/references/required-checks.md3.0 KB

security/clawdstrike/references/channels.md1.5 KB

security/clawdstrike/references/canvas-browser.md1.2 KB

security/clawdstrike/references/report-format.md1.3 KB

security/clawdstrike/references/evidence-template.md843 B

security/clawdstrike/references/discovery.md974 B

security/clawdstrike/references/verified-allowlist.md1.9 KB

security/clawdstrike/references/version-risk.md761 B

security/clawdstrike/references/redaction.md559 B

security/clawdstrike/references/gateway.md1.4 KB

security/clawdstrike/references/filesystem.md1.5 KB

security/clawdstrike/references/tools.md1.0 KB

security/clawdstrike/README.md1.7 KB

security/clawdstrike/scripts/config\_summary.py12 KB

security/clawdstrike/scripts/redact\_helpers.sh1.2 KB

security/clawdstrike/scripts/collect\_verified.sh14 KB

security/clawdstrike/SKILL.md3.5 KB

security/skillguard/RED-TEAM-NOTES.md4.8 KB

security/skillguard/README.md7.1 KB

security/skillguard/package.json411 B

security/skillguard/SKILL.md1.4 KB

security/skillguard/rules/dangerous-patterns.json5.9 KB

security/skillguard/src/ast-analyzer.js22 KB

security/skillguard/src/reporter.js6.5 KB

security/skillguard/src/scanner.js31 KB

security/skillguard/src/index.js1.7 KB

security/skillguard/src/prompt-analyzer.js21 KB

security/skillguard/src/cli.js5.2 KB

security/skillguard/src/clawhub.js1.9 KB

CLAWHUB\_WARNING.md1.4 KB

references/api.md8.2 KB

references/acquiring-mor.md4.3 KB

references/troubleshooting.md6.2 KB

references/economics.md5.7 KB

references/models.md3.8 KB

website/index.html26 KB

README.md14 KB

scripts/chat.sh4.6 KB

scripts/everclaw-wallet.mjs14 KB

scripts/morpheus-proxy.mjs15 KB

scripts/router.mjs14 KB

scripts/install.sh5.8 KB

scripts/balance.sh2.7 KB

scripts/install-proxy.sh4.5 KB

scripts/stop.sh1.0 KB

scripts/swap.sh6.4 KB

scripts/gateway-guardian.sh11 KB

scripts/install-everclaw.sh7.2 KB

scripts/x402-client.mjs15 KB

scripts/start.sh3.9 KB

scripts/bootstrap-gateway.mjs12 KB

scripts/agent-registry.mjs13 KB

scripts/session.sh4.7 KB
